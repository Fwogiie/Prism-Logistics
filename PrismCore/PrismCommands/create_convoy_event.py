...
@_create.subcommand(name="convoy", description="KEEP IN MIND, TIME HAS TO BE UNIX TIMESTAMP, AND DLCS SHOULD BE SEPERATED WITH '-'")
async def _create_convoy(ctx, discord_event_id: str, convoy_url: str, slot: str, slot_img: str=None):
    ...
    await discord_event.edit(name=req["name"],
                             description=f"- TruckersMP event: {convoy_url}\n- Server: {req['server']['name']}\n"
                                         ...
                                         f"- Our slot: {slot} - Slot image: [image]({slot_img})\n"
